...
def _add_momentum(df: pd.DataFrame, period: int) -> pd.Series:
...
def _add_willr(df: pd.DataFrame, period: int) -> pd.Series:
...
def _add_mfi(df: pd.DataFrame, period: int) -> pd.Series:
...
def _add_bbands(df: pd.DataFrame, period: int) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        f"BBANDS_{period}_upper": upper,
        f"BBANDS_{period}_middle": middle,
        f"BBANDS_{period}_lower": lower,
    })
...
def _apply_indicator(df: pd.DataFrame, indicator: str) -> pd.DataFrame:
    ...
    elif indicator.startswith("MFI_"):
        period = int(indicator.split("_", maxsplit=1)[1])
        df[indicator] = _add_mfi(df, period)
